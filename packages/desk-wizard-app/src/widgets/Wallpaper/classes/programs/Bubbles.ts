  ...
  override updateShared(): void {
    ...
    for (const ball of this.balls) {
      ...
      for (let i = 0; i < this.boundingPolygon.length; i++) {
        ...
        const dotAB = lineVector[0] * ballVector[0] + lineVector[1] * ballVector[1];
        const dotBB = lineVector[0] * lineVector[0] + lineVector[1] * lineVector[1];
        ...
        const ortho = {
          x: ball.x - projection.x,
          y: ball.y - projection.y,
        };
...
class BubblesControl extends ProgramControl2D<BubblesState> {
  ...
  override draw(): void {
    ...
    for (const ball of this.sharedState.balls) {
      ...
      this.ctx.beginPath();
      this.ctx.arc(ball.x * size.w, ball.y * size.h, ball.r * size.w, 0, 2 * Math.PI);
...
export const Bubbles = {
  create: (bubbleNumber: number): CanvasProgram<BubblesState, "per-screen"> => ({
    createState: (screens: ScreenInfo[]) =>
      new BubblesState(screens, { animate: true, fps: 60 }, bubbleNumber),
    createControl: (canvas: HTMLCanvasElement, state: BubblesState, screenIdx: number) =>
