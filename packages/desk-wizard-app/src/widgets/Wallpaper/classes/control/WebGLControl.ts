...
> extends SpanningControl<TState> {
  ...
  constructor(override canvas: HTMLCanvasElement, override sharedState: TState) {
    ...
    this.bufferInfo = twgl.createBufferInfoFromArrays(this.gl, sharedState.meshArrays);
  }
  ...
  override draw(): void {
    for (const screen of this.sharedState.screens) {
      this.gl.scissor(
        screen.scaledRect.x,
        this.sharedState.scaledSize.h - screen.scaledRect.y - screen.scaledRect.h,
        screen.scaledRect.w,
        screen.scaledRect.h,
      );
